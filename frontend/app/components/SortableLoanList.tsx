...
function SortableItem({ id, item, onEdit, onDelete }: SortableItemProps) {
  ...
  const formatDateRange = () => {
    const start = `${MONTHS[item.emiStartMonth].slice(0, 3)} ${item.emiStartYear}`;
    const end = `${MONTHS[item.emiEndMonth].slice(0, 3)} ${item.emiEndYear}`;
    return `${start} - ${end}`;
  };
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-gray-500 flex flex-wrap gap-x-2">
          ...
          <span>•</span>
          <span>EMI: {formatCurrency(item.emiValue)}</span>
          <span>•</span>
...
interface Props {
  items: Loan[];
  onReorder: (items: Loan[]) => void;
  onEdit: (item: Loan) => void;
  onDelete: (id: string) => void;
}

export default function SortableLoanList({ items, onReorder, onEdit, onDelete }: Props) {
